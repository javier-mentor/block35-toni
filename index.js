...
app.delete('/app/user/:user_id/userFavorite/:id', async(req,res,next)=>{
    try{
        await deleteUserFavorite({ user_id: req.params.user_id, id: req.params.id});
        res.sendStatus(204);
    }
    catch(ex){
        next(ex);
...
app.post('/app/user/:id/userFavorite', async(req,res,next)=>{
    try{
        res.status(201).send(await createUserFavorite({user_id: req.params.id, id: req.body.product_id}));
    }
    catch(ex){
        next(ex);
    }
...
const init = async()=> {
    ...
    const [Tom, Steve, Bob, Eugene, chair, desk, lamp ]= await Promise.all([
        createUser({ username1: 'Tom', password: 'Tom_pw'}),
        createUser({ username1: 'Steve', password: 'Steve_pw'}),
        createUser({ username1: 'Bob', password: 'Bob_pw'}),
        createUser({ username1: 'Eugene', password: 'Eugene_pw'}),
